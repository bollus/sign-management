...
//                    throw new ADIException("不存在的数据源："+datasource,500);
    ...
    // 创建数据源
    public boolean createDataSource(Long key, String driveClass, String url, String username, String password, String databasetype) {
        try {
            try { // 排除连接不上的错误
...
//            HikariDataSource druidDataSource = new HikariDataSource();
            ...
            druidDataSource.setName(String.valueOf(key));
...
//            }
    ...
    public boolean delDataSources(Long dataSourceId) {
        ...
        if (dynamicTargetDataSources2.containsKey(dataSourceId)) {
            ...
            for (DruidDataSource l : druidDataSourceInstances) {
                if (String.valueOf(dataSourceId).equals(l.getName())) {
    ...
    public void createDataSourceWithCheck(ServerDataSource dataSource) throws Exception {
    ...
    private  void createDataSource(ServerDataSource dataSource) throws Exception {
        Long datasourceId = dataSource.getId();
